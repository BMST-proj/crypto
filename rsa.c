...
// PLEASE DON'T USE THAT FOR ANYTHING SERIOUS

void gen_keys(int up_bound, int low_bound, int *pub_exp, int *pub_prod, int *priv_key){
	int p, q, phi;
	p = gen_prime(up_bound, low_bound); 
	q = gen_prime(up_bound, low_bound); 
	*pub_prod = p*q;
	phi = *pub_prod -p-q+1;
...
char* decrypt_naive(int* msg, int msg_size, int priv_key, int pub_prod){ // Optimiser avec le CRT
